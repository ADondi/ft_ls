...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/25 12:32:46 by adondera          #+#    #+#             */
/*   Updated: 2018/03/25 12:32:48 by adondera         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (cur)
	{
		if ((dir = opendir(cur->content)) == NULL)
			errno != ENOTDIR ? basicerror("ft_ls: ", cur->content, 0) : \
				ft_list_push_back_mod(&file, cur->content, cur->content_size);
		else
		{
			ft_list_push_back_mod(&directory, cur->content, cur->content_size);
			if (closedir(dir) == -1)
